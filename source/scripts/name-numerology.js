...
const calculateSoulUrgeNumber = function(name) {
  ...
  while (soulUrgeNumber > 9) {
    ...
    soulUrgeNumber = digits.reduce((sum, digit) => {
...
function saveUserName() {
  ...
  // Get the Soul Urge Number information from the data
  const info = soulUrgeNumberData[soulUrgeNumber];

  // Update the HTML with the Soul Urge Number information
  document.getElementById('soul-urge-number').textContent = soulUrgeNumber;
  document.getElementById('soul-urge-meaning').textContent = info.meaning;
  document.getElementById('soul-urge-description').textContent = info.description;
